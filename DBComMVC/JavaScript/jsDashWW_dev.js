...
var item = [];
var canaisLP = ["000001", "000002", "000003", "000004", "000005", "000006", "000007", "000008", "000009", "000010", "000011", "000021", "000023","000030","000031","000032","000033","000036","000039","000040","000041","000042","000045","000046"];
var canaisWW = []; //"000012", "000013", "000014", "000015", "000016", "000017", "000018", "000019", "000020", "000022", "000024", "000025", "000026","000030","000031","000032","000033","000036","000041","000042","000039","000040"
...
function AtualizaCamposGerais() {
    $.getJSON("/rest/LPDASH/PREVISTO").done(function (data) {		
        aFaturarGeralMarca = 0;
        $.each(data.resultado, function (key, equipe) {
        ...
        for (i = 0; i < data.resultado.length; i++) {
            item = data.resultado[i];
            if (codGerenteAtualEquipe === item.COD_GERENTE_CANAL && canaisLP.indexOf(item.COD_CANAL) > -1) {
        ...
        $('#aFaturarParcialPerc').fadeOut('slow').text(Math.round10(aFaturarParcialPerc, -2) + "%").fadeIn('slow');
        aFaturarEquipe = 0;
    })
    $.getJSON("/rest/LPDASH/CONSOLIDADO").done(function (data) {
        ArrConsolidadoDia = [];
        valConsolidadoDia = 0;
...
function AtualizaResultadoCanaisEquipes() {
    $.getJSON("/rest/LPDASH/CANAIS").done(function (data) {
        qtdEquipes = (data.resultado.length - 1);
        ...
        $.each(data.resultado, function (key, equipe) {
            if (canaisLP.indexOf(equipe.COD_CANAL) > -1) {
                totalMarca += parseInt(equipe.TT_CTGs_VENDA.replace('R$ ', '').replace('.', '').replace('.', '').replace('.', '').replace(',', ''));
            }
        });
        $.each(data.resultado, function (key, equipe) {
            if (canaisWW.indexOf(equipe.COD_CANAL) > -1) {
                totalMarca += parseInt(equipe.TT_MARCA_WW.replace('R$ ', '').replace('.', '').replace('.', '').replace('.', '').replace(',', ''));
            }
        }
        );
            ...
            if (canaisLP.indexOf(equipe.COD_CANAL) > -1) {
                if (arrayCodsGerentes.indexOf(equipe.COD_GERENTE_CANAL) == -1) {
                    ...
                    if (equipe.COD_GERENTE_CANAL == codGerenteAtual) {
						...
						if((equipe.PERC_STATUS_META * 100)>100.00){
							percMetaLength = "100";
						}
						else{
                        ...
                        }).text(Math.round10(percMeta.replace(" %", ""), -2) + "%")), $('<td></td>').append($('<div>', {
                            style: 'margin: 6px 0px;'
                        }).append($('<p>', {
                            style: 'font-size:16px; font-weight:400; color:#515151; margin:0px;'
                        }).text('R$ '+equipe.TT_META_MES))), $('<td></td>').append($('<div>', {
                            style: 'margin: 6px 0px;'
                        }).append($('<p>', {
                            style: 'font-size:16px; font-weight:400; color:#515151; margin:0px;'
                        }).text('R$ '+equipe.TT_CTGs_VENDA))))).fadeIn('slow');
        ...
        $('#percResultaVsMetaTT').fadeOut('slow').text(Math.round10(perc_ttMetaGeralEquipes, -2) + "% ").fadeIn('slow');
    })
}
...
function AtualizacaoDatasConsulta() {
    $.getJSON("/rest/LPDASH/PERIODO").done(function (data) {
